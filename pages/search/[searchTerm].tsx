...
const Search = ({ videos }: { videos: Video[] }) => {
  ...
  const accounts = isAccounts ? 'border-b-2 border-red-200 text-red-300' : 'text-gray-400';
  const rants = !isAccounts ? 'border-b-2 border-red-200 text-red-300' : 'text-gray-400';
  ...
  return (
    <div className='w-full  '>
      <div className='flex gap-10 mb-10 border-b-2 border-gray-200 md:fixed z-50 bg-white w-full'>
        ...
        <p className={`text-xl font-semibold cursor-pointer ${rants} mt-2`} onClick={() => setIsAccounts(false)}>
          Rants
        </p>
      </div>
      {isAccounts ? (
